...
/**
 * 单例模式
 * 定义：这个类在程序中只能被实例化一次，它提供了一种访问其唯一的对象的方式，避免一个全局使用的类频繁地创建与销毁。
...
/**
 * 注册树模式
 * 定义：注册树模式通过将对象实例注册到一棵全局的对象树上，需要的时候从对象树上采摘的模式设计方法。
...
/**
 * 适配器模式
 * 定义：将各种截然不同的函数接口封装成统一的API，根据调用时传参不同而适配选择不同的逻辑代码。
...
/**
 * 观察者模式
 * 定义:观察者模式(Observer)，当一个对象状态发生变化时，依赖它的对象全部会收到通知，并自动更新。观察者模式实现了低耦合，非侵入式的通知与更新机制。
